...
idmrg(MPS & psi, 
    ...
    auto saveFileName = args.getString("IntermediateSaveFileName");
    ...
    for(; sw <= sweeps.nsweep(); ++sw)
        ...
        if(fileExists("WRITE_WF") && sw==sweeps.nsweep())
            {
            ...
            // writeToFile(format("sites_%d",sw),wpsi.sites());
            }
